...
class DeviceReminder extends utils.Adapter {
    ...
    async onReady() {
        ...
        // for (const i of Object.keys(this.config.telegram)) {

        //     const parsedData = JSON.parse(this.config.telegram[i].user.replace(/\\/g, ''))

        //     this.telegram[this.config.telegram[i].id] = {
        //         name : parsedData.name,
        //         chatID : parsedData.ChatID,
        //         instance : parsedData.instance,
        //         active : this.config.telegram[i].active,
        //     };
        // };
    ...
    async onMessage(obj) {
        ...
        switch (obj.command) {
            ...
            case "getInstance": // hier werden alle installierten Instanzen eines Adapters geholt und per selectSendTo in der Admin UI angezeigt
                ...
                this.respond(obj, await this.getInstance(obj.message.name), this);
...
this.log.info(JSON.stringify(obj))
                obj.message.name = obj.message.attr.replace("show", "").toLowerCase();

                const result = await this.getInstance(obj.message.name)
                this.log.info(`Es wurde(n) Instanzen fuer ${obj.message.name} gefunden. ${JSON.stringify(result)}`)
            ...
            case "test":
                ...
                this.log.warn(JSON.stringify(arrResult))
                this.respond(obj, arrResult, this);
    ...
    async respond(obj, response, that) {
        // this.log.warn('respond wird ausgefuehrt: ' + JSON.stringify(response));
        try {
