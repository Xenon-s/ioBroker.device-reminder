 ...
 * @returns {Promise<Array>} - Ein Promise, das das Array ohne den entfernten Wert enthält
 */
...
/**
 * Extrahiert alle "id"s aus einem Array von Objekten asynchron.
 * @param {Array} data - Das Eingabearray von Objekten.
 * @returns {Promise<Array>} Ein Promise, das ein Array von "id"s enthält.
 */
async function getIdFromObject(data) {
    try {
        const ids = [];

        // Iteration über jedes Objekt im Eingabearray
        data.forEach(item => {
            // Hinzufügen der ID des aktuellen Objekts zum Array der IDs
            ids.push(item.id);
        });
        return ids;
    } catch (error) {
        // Fangen und Weitergabe von Fehlern, falls sie auftreten
        throw new Error('Fehler beim Extrahieren der IDs: ' + error.message);
    };
};
